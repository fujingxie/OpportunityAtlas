...
8. **文档导入分流**：管理端上传接口 `POST /api/admin/import/jobs` 使用 `sourceType=program|case|mixed|unknown` 区分文档类型；活动文档解析为 `Program` 草稿，兼容 Markdown 编号标题和 `新增 N.` 活动标题，也允许字段标签中 `/` 两侧存在空格；案例库模板解析为 `StudentCase + CaseActivity` 草稿，`mixed/unknown` 暂不支持。
9. **筛选设计**：公开接口 `GET /api/tags` 只返回启用标签；活动库保留左侧传统筛选栏，并在侧栏顶部提供活动探索问卷辅助填充年级、申请方向、经历目标、形式、地点和费用条件；案例库为分步问答式筛选，按就读体系、标化成绩、语言成绩、竞赛、夏校、科研、申请地区和案例等级过滤。
...
14. **管理员登录入口**：公共站点头部不再提供管理员登录按钮；管理员直接访问 `/admin`，由管理端登录页完成邮箱/密码登录，登录后退出入口位于数据管理侧栏底部。
15. **路径规划一期**：公开页 `/planner` 提供三步问卷，接口 `POST /api/planner/recommendations` 基于内部活动库和案例库生成推荐活动、相似案例、阶段路径和解释理由；首页搜索、活动库/详情、案例库/详情可通过 `q`、`programId`、`caseId` 作为路径规划上下文入口，服务端会将当前活动、参考案例或搜索关键词作为推荐锚点；结果页按阶段分组展示活动，并输出优先级、适配摘要、注意事项、下一步动作、风险提示和参考案例；解释层已提供 `advisorExplanation` AI-ready 结构，支持 `PLANNER_EXPLANATION_PROVIDER=template|deepseek`，DeepSeek 只替换顾问式解释表达，失败时回退模板，不改排序、风险提示和事实字段；一期使用内部规则，不做外部联网搜索。
16. **路径规划体验增强**：规划问卷左侧显示当前画像摘要，背景经历支持“暂无/基础/较强”快捷填充；结果页优先展示“优先补强、首选活动、参考案例”结论卡，再展示顾问解释与系统规则说明；活动详情页新增“适合谁 / 不适合谁 / 准备建议”，帮助用户在进入规划前判断活动适配边界。DeepSeek 解释层提示词要求点名活动/案例、输出 JSON，并关闭 thinking 模式以减少非结构化输出。
17. **管理后台体验方向**：后台从“大圆角容器 + 页面内散卡片”调整为固定深色侧栏 + 右侧工作区；活动、案例、标签、关联管理采用指标条、工具栏、状态筛选、搜索和紧凑表格优先的维护模式；新增/编辑统一使用右侧抽屉，减少页面跳转和上下滚动成本。文档录入页上传区压缩为工具栏式，队列突出当前选中任务。
